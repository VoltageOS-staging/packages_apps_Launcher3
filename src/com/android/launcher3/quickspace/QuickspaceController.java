...
public class QuickspaceController implements OmniJawsClient.OmniJawsObserver,
    ...
    private void initializeWeatherIfNeeded() {
        if (!mWeatherInitialized && LauncherPrefs.SHOW_QUICKSPACE_WEATHER.get(mContext)) {
    ...
    private void initializeMediaIfNeeded() {
        if (!mMediaInitialized && LauncherPrefs.SHOW_QUICKSPACE_NOWPLAYING.get(mContext)) {
    ...
    public String getWeatherTemp() {
        ...
        // Run expensive string building on background thread
        MODEL_EXECUTOR.execute(() -> {
            ...
            String result = weatherTemp.toString();
            
            // Update cache on main thread
            MAIN_EXECUTOR.execute(() -> {
                mCachedWeatherTemp = result;
                mWeatherCacheTime = System.currentTimeMillis();
            });
        });

        // Return old cache or null while new result is being computed
        return mCachedWeatherTemp;
    }
